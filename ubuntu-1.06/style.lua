        ...
        if style.dash then
            ...
            if style.dash.array then
                ...
                for i,v in ipairs(style.dash.array) do
                    assert(type(v) == "number", "dash array entry not number")
                    assert(v > 0, "invalid dash length")
                end
            end
        end
    else
...
function _M.copy(style)
    ...
    if type(style) == "table" then
        local copy = {}
        if style.dash then
            -- zero initial phase if none specified
            copy.dash = { initial_phase = style.dash.initial_phase or 0 }
            if style.dash.array then
                copy.dash.array = {}
                local n = #style.dash.array
                for i = 1, n do
                    copy.dash.array[i] = style.dash.array[i]
                end
                -- double size if not even
                if (n % 2) ~= 0 then
                    for i = 1, n do
                        copy.dash.array[i+n] = copy.dash.array[i]
                    end
                end
            end
        end
