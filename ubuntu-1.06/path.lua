...
local rdata = {}
function rdata.begin_open_contour(len, x0, y0)
    return x0, y0, len
end
rdata.begin_closed_contour = rdata.begin_open_contour
function rdata.end_open_contour(x0, y0, len)
    return len, x0, y0
end
rdata.end_closed_contour = rdata.end_open_contour
function rdata.linear_segment(x0, y0, x1, y1)
    return x1, y1, x0, y0
end
function rdata.linear_segment_with_len(x0, y0, len, x1, y1)
    return x1, y1, len, x0, y0
end
function rdata.quadratic_segment(x0, y0, x1, y1, x2, y2)
    return x2, y2, x1, y1, x0, y0
end
function rdata.rational_quadratic_segment(x0, y0, x1, y1, w1, x2, y2)
    return x2, y2, x1, y1, w1, x0, y0
end
function rdata.cubic_segment(x0, y0, x1, y1, x2, y2, x3, y3)
    return x3, y3, x2, y2, x1, y1, x0, y0
end
function rdata.begin_segment(s, t)
    return s, t
end
rdata.end_segment = rdata.begin_segment

-- iterate over path contents in reverse order
function path_meta.__index.riterate(path, forward)
    for index = #path.instructions, 1, -1 do
        local instruction = path.instructions[index]
        local offset = path.offsets[index]
        -- invoke method with same name as
        -- instruction from forward table, passing all its data
        local reverse = rdata[instruction]
        local callback = assert(forward[rinstruction[instruction]],
            "unhandled instruction '" .. instruction .. "'")
        callback(forward,
            reverse(unpack(path.data, offset, offset+ndata[instruction])))
    end
end
