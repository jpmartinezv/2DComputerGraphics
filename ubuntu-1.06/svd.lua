...
local function hypot(x, y)
    x = math.abs(x)
    y = math.abs(y)
    if x < FLT_MIN and y < FLT_MIN then return 0 end
...
function _M.us(a, b, c, d)
    ...
    if s0 > FLT_MIN then -- at least 1 singular value above threshold
        ...
        if s1 > FLT_MIN then -- both singular values are above threshold
