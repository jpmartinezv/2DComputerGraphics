...
function _M.degenerate(x0, y0, rx, ry, rot_deg, fa, fs, x2, y2)
    local dx = x2-x0
    local dy = y2-y0
    return dx*dx+dy*dy < FLT_MIN*FLT_MIN or rx < FLT_MIN or ry < FLT_MIN
end
...
function _M.torational(x0, y0, rx, ry, rot_deg, fa, fs, x2, y2)
    ...
    if rx < FLT_MIN or ry < FLT_MIN then
    ...
    -- split the arc into pieces that subintend at most Pi/2 radians
    if fa then
        return -x1, -y1, -w1
    else
        return x1, y1, w1
...
function _M.tosvg(x0, y0, x1, y1, w1, x2, y2)
    ...
    local s2 = 1-w1*w1
    assert(math.abs(s2) > FLT_MIN, "not an ellipse (parabola?)")
