...
function path_meta.__index.begin_open_contour(path, x0, y0)
    begin_contour(path, "begin_open_contour", x0, y0)
end

function path_meta.__index.begin_closed_contour(path, x0, y0)
...
function path_meta.__index.end_open_contour(path)
    return end_contour(path, "end_open_contour")
end

function path_meta.__index.end_closed_contour(path)
...
function path_meta.__index.linear_segment(path, x0, y0)
    push_instruction(path, "linear_segment")
    push_data(path, x0, y0)
end

function path_meta.__index.quadratic_segment(path, x0, y0, x1, y1)
    push_instruction(path, "quadratic_segment")
    push_data(path, x0, y0, x1, y1)
end

function path_meta.__index.rational_quadratic_segment(path, x0, y0, w0, x1, y1)
    push_instruction(path, "rational_quadratic_segment")
    push_data(path, x0, y0, w0, x1, y1)
end

function path_meta.__index.cubic_segment(path, x0, y0, x1, y1, x2, y2)
...
function append.rquad_to_abs(path, x0, y0, w0, x1, y1)
    ...
    path:rational_quadratic_segment(x0, y0, w0, x1, y1)
...
function append.cubic_to_abs(path, x0, y0, x1, y1, x2, y2)
    ensure_non_empty(path)
    pop_end_contour_sentinel(path)
    path:cubic_segment(x0, y0, x1, y1, x2, y2)
    push_end_contour_sentinel(path)
    set_previous(path, x1, y1)
...
function append.quad_to_abs(path, x0, y0, x1, y1)
    ensure_non_empty(path)
    pop_end_contour_sentinel(path)
    path:quadratic_segment(x0, y0, x1, y1)
    push_end_contour_sentinel(path)
    set_previous(path, x0, y0)
    set_current(path, x1, y1)
end
